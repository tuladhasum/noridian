...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.example.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.examples.update", [$example->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('secret') ? 'has-error' : '' }}">
                ...
                @foreach(App\Example::SECRET_RADIO as $key => $label)
                    <div>
                        <input id="secret_{{ $key }}" name="secret" type="radio" value="{{ $key }}" {{ old('secret', $example->secret) === (string)$key ? 'checked' : '' }}>
            ...
            <div class="form-group {{ $errors->has('meal_preference') ? 'has-error' : '' }}">
                ...
                <select id="meal_preference" name="meal_preference" class="form-control">
                    ...
                    @foreach(App\Example::MEAL_PREFERENCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('meal_preference', $example->meal_preference) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('terms_agreement') ? 'has-error' : '' }}">
                ...
                <input value="1" type="checkbox" id="terms_agreement" name="terms_agreement" {{ (isset($example) && $example->terms_agreement) || old('terms_agreement', 0) === 1 ? 'checked' : '' }}>
